...
/**
 * Font handling moved to `./printFonts` (RAISE-53) so both PDF and
 * HTML export can share the @font-face substitution pipeline.
 * `injectPrintFonts(html)` returns the HTML with the placeholder
 * marker replaced by the actual @font-face block.
 */
 ...
 */
function pageSizeArg(size: ExportPdfOptions['pageSize']): PrintToPDFOptions['pageSize'] {
...
function buildSlotTemplate(
  ...
  const substituteUserValues = (s: string): string =>
    s.replace(/\{author\}/g, author).replace(/\{email\}/g, email);
...
export async function exportToPdf(
  ...
  try {
    ...
    tempHtmlPath = path.join(tempDir, `print-${randomUUID()}.html`);
    ...
    // font assets reliably. Shared with HTML export via the
    // injectPrintFonts helper (RAISE-53).
    let finalHtml: string;
    try {
      finalHtml = await injectPrintFonts(opts.html);
    } catch (err) {
      ...
      console.error('[exportPdf] injectPrintFonts failed:', message);
      return {
        status: 'error',
    ...
    if (opts.headerFooter) {
      ...
      printOpts.headerTemplate = hf.showHeader
        ? buildSlotTemplate(hf.headerLeft, hf.headerCenter, hf.headerRight, hf.author, hf.email)
        : '<div></div>';
      printOpts.footerTemplate = hf.showFooter
        ? buildSlotTemplate(hf.footerLeft, hf.footerCenter, hf.footerRight, hf.author, hf.email)
        : '<div></div>';
    }
