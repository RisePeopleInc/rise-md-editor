...
- **Windows installers are now signed via Azure Trusted Signing** ([RAISE-58](https://risepeople.atlassian.net/browse/RAISE-58)). SmartScreen no longer shows the "Unknown publisher" warning on first install — the signed `.exe` chains up to Microsoft's trusted root and shows _Rise People Inc._ as the verified publisher. Signing happens during the CI build via electron-builder's `signtoolOptions.sign` callback ([`scripts/sign-windows.cjs`](scripts/sign-windows.cjs)) shelling out to Microsoft's Trusted Signing dlib + `signtool.exe`. The cert lives in Microsoft's FIPS 140-2 Level 3 HSM and never touches the runner; the runner authenticates to Azure via OIDC federation (no long-lived service-principal secret). Cost is ~$10/month (Trusted Signing Basic tier). The `build-win` job declares `environment: rise-md-editor-signing` so only authorized workflow runs can exchange their OIDC token for the cert-profile signing role.
