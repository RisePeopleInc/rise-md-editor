...
- **Milkdown plugins can fire init-time transactions that look like user edits.** GFM linkify (during parse) and `autolinkOnTypePlugin` / `stripBrowserAutolinkPlugin` / `trailingParagraphPlugin` (via `appendTransaction` on the first docChanged) emit `markdownUpdated` events before any user input — with markdown that differs from the on-disk source. Dirty tracking in `fileState` uses a per-tab `editorBaseline` that captures the editor's first post-load emit as the comparison baseline, so cosmetic round-trip drift (autolinks added to bare URLs, list-marker normalization, etc.) doesn't flip `isDirty` (RAISE-55). When adding a new appendTransaction plugin, this still works as long as it stays in WYSIWYG mode and fires via the listener; if you wire a new code path that mutates content outside of `setContent`, make sure it updates `editorBaseline` too or it'll cause spurious dirty signals.
