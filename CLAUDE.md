...
- **Two Windows installers, and the MSI must not auto-update** (RAISE-90). Releases ship a per-user NSIS `.exe` (auto-updates) _and_ a per-machine `.msi` for Intune deployment. Both targets package the _same_ app bundle from one `build:win`, so you can't flip a build-time flag per target — `autoUpdater.ts` detects the managed install at runtime (`isManagedDeployment`: on Windows, the install dir under `Program Files` isn't user-writable) and skips update checks. If you touch the updater, preserve that gate; an MSI install that tries to self-update would just nag with banners a standard user can't action while fighting Intune. The MSI is for managed deployment only — see `docs/release-process.md`.
