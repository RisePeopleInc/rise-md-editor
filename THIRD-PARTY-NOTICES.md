# Third-party notices

Rise MD Editor bundles open-source software. This file lists every production dependency in the npm tree along with its license, repository, and publisher. Auto-generated from `package-lock.json` — see the regeneration command below.

To regenerate after a dependency change:

```sh
node scripts/regenerate-third-party-notices.cjs
```

**Total production packages: 300**

## License summary

| License | Count |
| --- | --- |
| `(MIT AND Zlib)` | 1 |
| `(MIT OR CC0-1.0)` | 1 |
| `(MIT OR GPL-3.0-or-later)` | 1 |
| `(MPL-2.0 OR Apache-2.0)` | 1 |
| `Apache-2.0` | 1 |
| `BSD-2-Clause` | 4 |
| `BSD-3-Clause` | 2 |
| `BlueOak-1.0.0` | 1 |
| `ISC` | 8 |
| `MIT` | 277 |
| `OFL-1.1` | 2 |
| `Python-2.0` | 1 |

All licenses listed above are permissive and MIT-compatible. Dual-licensed packages (e.g. `MIT OR GPL-3.0-or-later`, `MPL-2.0 OR Apache-2.0`, `MIT OR CC0-1.0`) are consumed under their permissive option. `OFL-1.1` covers font files only and does not infect source code that uses them — see `docs/license-rationale.md` for the analysis.

## Packages

Listed alphabetically within each license group. Each entry: name@version, license, publisher (if known), repository.

### (MIT AND Zlib) (1)

- **pako@1.0.11** — `(MIT AND Zlib)`
  - https://github.com/nodeca/pako

### (MIT OR CC0-1.0) (1)

- **type-fest@2.19.0** — `(MIT OR CC0-1.0)` — Sindre Sorhus
  - https://github.com/sindresorhus/type-fest

### (MIT OR GPL-3.0-or-later) (1)

- **jszip@3.10.1** — `(MIT OR GPL-3.0-or-later)` — Stuart Knightley
  - https://github.com/Stuk/jszip

### (MPL-2.0 OR Apache-2.0) (1)

- **dompurify@3.4.5** — `(MPL-2.0 OR Apache-2.0)` — Dr.-Ing. Mario Heiderich, Cure53
  - https://github.com/cure53/DOMPurify

### Apache-2.0 (1)

- **typescript@5.9.3** — `Apache-2.0` — Microsoft Corp.
  - https://github.com/microsoft/TypeScript

### BSD-2-Clause (4)

- **@mixmark-io/domino@2.2.0** — `BSD-2-Clause` — Felix Gnass
  - https://github.com/mixmark-io/domino
- **entities@4.5.0** — `BSD-2-Clause` — Felix Boehm
  - https://github.com/fb55/entities
- **entities@7.0.1** — `BSD-2-Clause` — Felix Boehm
  - https://github.com/fb55/entities
- **json-schema-typed@7.0.3** — `BSD-2-Clause`
  - https://github.com/typeslick/json-schema-typed

### BSD-3-Clause (2)

- **fast-uri@3.1.2** — `BSD-3-Clause` — Vincent Le Goff
  - https://github.com/fastify/fast-uri
- **source-map-js@1.2.1** — `BSD-3-Clause` — Valentin 7rulnik Semirulnik
  - https://github.com/7rulnik/source-map-js

### BlueOak-1.0.0 (1)

- **sax@1.6.0** — `BlueOak-1.0.0` — Isaac Z. Schlueter
  - https://github.com/isaacs/sax-js

### ISC (8)

- **anymatch@3.1.3** — `ISC` — Elan Shanker
  - https://github.com/micromatch/anymatch
- **glob-parent@5.1.2** — `ISC` — Gulp Team
  - https://github.com/gulpjs/glob-parent
- **graceful-fs@4.2.11** — `ISC`
  - https://github.com/isaacs/node-graceful-fs
- **inherits@2.0.4** — `ISC`
  - https://github.com/isaacs/inherits
- **markdown-it-task-lists@2.1.1** — `ISC` — Revin Guillen
  - https://github.com/revin/markdown-it-task-lists
- **picocolors@1.1.1** — `ISC` — Alexey Raspopov
  - https://github.com/alexeyraspopov/picocolors
- **semver@7.7.4** — `ISC` — GitHub Inc.
  - https://github.com/npm/node-semver
- **semver@7.8.0** — `ISC` — GitHub Inc.
  - https://github.com/npm/node-semver

### MIT (277)

- **@babel/helper-string-parser@7.27.1** — `MIT` — The Babel Team
  - https://github.com/babel/babel
- **@babel/helper-validator-identifier@7.28.5** — `MIT` — The Babel Team
  - https://github.com/babel/babel
- **@babel/parser@7.29.3** — `MIT` — The Babel Team
  - https://github.com/babel/babel
- **@babel/types@7.29.0** — `MIT` — The Babel Team
  - https://github.com/babel/babel
- **@codemirror/autocomplete@6.20.2** — `MIT` — Marijn Haverbeke
  - git+https://code.haverbeke.berlin/codemirror/autocomplete
- **@codemirror/commands@6.10.3** — `MIT` — Marijn Haverbeke
  - https://github.com/codemirror/commands
- **@codemirror/lang-angular@0.1.4** — `MIT` — Marijn Haverbeke
  - https://github.com/codemirror/lang-angular
- **@codemirror/lang-cpp@6.0.3** — `MIT` — Marijn Haverbeke
  - https://github.com/codemirror/lang-cpp
- **@codemirror/lang-css@6.3.1** — `MIT` — Marijn Haverbeke
  - https://github.com/codemirror/lang-css
- **@codemirror/lang-go@6.0.1** — `MIT` — Marijn Haverbeke
  - https://github.com/codemirror/lang-go
- **@codemirror/lang-html@6.4.11** — `MIT` — Marijn Haverbeke
  - https://github.com/codemirror/lang-html
- **@codemirror/lang-java@6.0.2** — `MIT` — Marijn Haverbeke
  - https://github.com/codemirror/lang-java
- **@codemirror/lang-javascript@6.2.5** — `MIT` — Marijn Haverbeke
  - https://github.com/codemirror/lang-javascript
- **@codemirror/lang-jinja@6.0.1** — `MIT` — Marijn Haverbeke
  - git+https://code.haverbeke.berlin/codemirror/lang-jinja
- **@codemirror/lang-json@6.0.2** — `MIT` — Marijn Haverbeke
  - https://github.com/codemirror/lang-json
- **@codemirror/lang-less@6.0.2** — `MIT` — Marijn Haverbeke
  - https://github.com/codemirror/lang-less
- **@codemirror/lang-liquid@6.3.2** — `MIT` — Marijn Haverbeke
  - https://github.com/codemirror/lang-liquid
- **@codemirror/lang-markdown@6.5.0** — `MIT` — Marijn Haverbeke
  - https://github.com/codemirror/lang-markdown
- **@codemirror/lang-php@6.0.2** — `MIT` — Marijn Haverbeke
  - https://github.com/codemirror/lang-php
- **@codemirror/lang-python@6.2.1** — `MIT` — Marijn Haverbeke
  - https://github.com/codemirror/lang-python
- **@codemirror/lang-rust@6.0.2** — `MIT` — Marijn Haverbeke
  - https://github.com/codemirror/lang-rust
- **@codemirror/lang-sass@6.0.2** — `MIT` — Marijn Haverbeke
  - https://github.com/codemirror/lang-sass
- **@codemirror/lang-sql@6.10.0** — `MIT` — Marijn Haverbeke
  - https://github.com/codemirror/lang-sql
- **@codemirror/lang-vue@0.1.3** — `MIT` — Marijn Haverbeke
  - https://github.com/codemirror/lang-vue
- **@codemirror/lang-wast@6.0.2** — `MIT` — Marijn Haverbeke
  - https://github.com/codemirror/lang-wast
- **@codemirror/lang-xml@6.1.0** — `MIT` — Marijn Haverbeke
  - https://github.com/codemirror/lang-xml
- **@codemirror/lang-yaml@6.1.3** — `MIT` — Marijn Haverbeke
  - https://github.com/codemirror/lang-yaml
- **@codemirror/language-data@6.5.2** — `MIT` — Marijn Haverbeke
  - https://github.com/codemirror/language-data
- **@codemirror/language@6.12.3** — `MIT` — Marijn Haverbeke
  - https://github.com/codemirror/language
- **@codemirror/legacy-modes@6.5.3** — `MIT` — Marijn Haverbeke
  - git+https://code.haverbeke.berlin/codemirror/legacy-modes
- **@codemirror/lint@6.9.6** — `MIT` — Marijn Haverbeke
  - git+https://code.haverbeke.berlin/codemirror/lint
- **@codemirror/search@6.7.0** — `MIT` — Marijn Haverbeke
  - git+https://code.haverbeke.berlin/codemirror/search
- **@codemirror/state@6.6.0** — `MIT` — Marijn Haverbeke
  - https://github.com/codemirror/state
- **@codemirror/theme-one-dark@6.1.3** — `MIT` — Marijn Haverbeke
  - https://github.com/codemirror/theme-one-dark
- **@codemirror/view@6.43.0** — `MIT` — Marijn Haverbeke
  - git+https://code.haverbeke.berlin/codemirror/view
- **@floating-ui/core@1.7.5** — `MIT` — atomiks
  - https://github.com/floating-ui/floating-ui
- **@floating-ui/dom@1.7.6** — `MIT` — atomiks
  - https://github.com/floating-ui/floating-ui
- **@floating-ui/utils@0.2.11** — `MIT` — atomiks
  - https://github.com/floating-ui/floating-ui
- **@jridgewell/sourcemap-codec@1.5.5** — `MIT` — Justin Ridgewell
  - https://github.com/jridgewell/sourcemaps
- **@lezer/common@1.5.2** — `MIT` — Marijn Haverbeke
  - https://github.com/lezer-parser/common
- **@lezer/cpp@1.1.5** — `MIT` — Marijn Haverbeke
  - https://github.com/lezer-parser/cpp
- **@lezer/css@1.3.3** — `MIT` — Marijn Haverbeke
  - https://github.com/lezer-parser/css
- **@lezer/go@1.0.1** — `MIT` — Marijn Haverbeke
  - https://github.com/lezer-parser/go
- **@lezer/highlight@1.2.3** — `MIT` — Marijn Haverbeke
  - https://github.com/lezer-parser/highlight
- **@lezer/html@1.3.13** — `MIT` — Marijn Haverbeke
  - https://github.com/lezer-parser/html
- **@lezer/java@1.1.3** — `MIT` — Marijn Haverbeke
  - https://github.com/lezer-parser/java
- **@lezer/javascript@1.5.4** — `MIT` — Marijn Haverbeke
  - https://github.com/lezer-parser/javascript
- **@lezer/json@1.0.3** — `MIT` — Arun Srinivasan
  - https://github.com/lezer-parser/json
- **@lezer/lr@1.4.10** — `MIT` — Marijn Haverbeke
  - git+https://code.haverbeke.berlin/lezer/lr
- **@lezer/markdown@1.6.3** — `MIT` — Marijn Haverbeke
  - https://github.com/lezer-parser/markdown
- **@lezer/php@1.0.5** — `MIT` — Marijn Haverbeke
  - https://github.com/lezer-parser/php
- **@lezer/python@1.1.18** — `MIT` — Marijn Haverbeke
  - https://github.com/lezer-parser/python
- **@lezer/rust@1.0.2** — `MIT` — Marijn Haverbeke
  - https://github.com/lezer-parser/rust
- **@lezer/sass@1.1.0** — `MIT` — Marijn Haverbeke
  - https://github.com/lezer-parser/sass
- **@lezer/xml@1.0.6** — `MIT` — Marijn Haverbeke
  - https://github.com/lezer-parser/xml
- **@lezer/yaml@1.0.4** — `MIT` — Marijn Haverbeke
  - https://github.com/lezer-parser/yaml
- **@marijn/find-cluster-break@1.0.2** — `MIT` — Marijn Haverbeke
  - https://github.com/marijnh/find-cluster-break
- **@milkdown/components@7.21.1** — `MIT`
  - https://github.com/Milkdown/milkdown
- **@milkdown/core@7.21.1** — `MIT`
  - https://github.com/Milkdown/milkdown
- **@milkdown/crepe@7.21.1** — `MIT`
  - https://github.com/Milkdown/milkdown
- **@milkdown/ctx@7.21.1** — `MIT`
  - https://github.com/Milkdown/milkdown
- **@milkdown/exception@7.21.1** — `MIT`
  - https://github.com/Milkdown/milkdown
- **@milkdown/kit@7.21.1** — `MIT`
  - https://github.com/Milkdown/milkdown
- **@milkdown/plugin-block@7.21.1** — `MIT`
  - https://github.com/Milkdown/milkdown
- **@milkdown/plugin-clipboard@7.21.1** — `MIT`
  - https://github.com/Milkdown/milkdown
- **@milkdown/plugin-cursor@7.21.1** — `MIT`
  - https://github.com/Milkdown/milkdown
- **@milkdown/plugin-diff@7.21.1** — `MIT`
  - https://github.com/Milkdown/milkdown
- **@milkdown/plugin-history@7.21.1** — `MIT`
  - https://github.com/Milkdown/milkdown
- **@milkdown/plugin-indent@7.21.1** — `MIT`
  - https://github.com/Milkdown/milkdown
- **@milkdown/plugin-listener@7.21.1** — `MIT`
  - https://github.com/Milkdown/milkdown
- **@milkdown/plugin-slash@7.21.1** — `MIT`
  - https://github.com/Milkdown/milkdown
- **@milkdown/plugin-streaming@7.21.1** — `MIT`
  - https://github.com/Milkdown/milkdown
- **@milkdown/plugin-tooltip@7.21.1** — `MIT`
  - https://github.com/Milkdown/milkdown
- **@milkdown/plugin-trailing@7.21.1** — `MIT`
  - https://github.com/Milkdown/milkdown
- **@milkdown/plugin-upload@7.21.1** — `MIT`
  - https://github.com/Milkdown/milkdown
- **@milkdown/preset-commonmark@7.21.1** — `MIT`
  - https://github.com/Milkdown/milkdown
- **@milkdown/preset-gfm@7.21.1** — `MIT`
  - https://github.com/Milkdown/milkdown
- **@milkdown/prose@7.21.1** — `MIT`
  - https://github.com/Milkdown/milkdown
- **@milkdown/react@7.21.1** — `MIT`
  - https://github.com/Milkdown/milkdown
- **@milkdown/theme-nord@7.21.1** — `MIT`
  - https://github.com/Milkdown/milkdown
- **@milkdown/transformer@7.21.1** — `MIT`
  - https://github.com/Milkdown/milkdown
- **@milkdown/utils@7.21.1** — `MIT`
  - https://github.com/Milkdown/milkdown
- **@monaco-editor/loader@1.7.0** — `MIT` — Suren Atoyan
  - https://github.com/suren-atoyan/monaco-loader
- **@monaco-editor/react@4.7.0** — `MIT` — Suren Atoyan
  - https://github.com/suren-atoyan/monaco-react
- **@ocavue/utils@1.6.0** — `MIT` — ocavue
  - https://github.com/ocavue/utils
- **@types/debug@4.1.13** — `MIT`
  - https://github.com/DefinitelyTyped/DefinitelyTyped
- **@types/hast@3.0.4** — `MIT`
  - https://github.com/DefinitelyTyped/DefinitelyTyped
- **@types/katex@0.16.8** — `MIT`
  - https://github.com/DefinitelyTyped/DefinitelyTyped
- **@types/lodash-es@4.17.12** — `MIT`
  - https://github.com/DefinitelyTyped/DefinitelyTyped
- **@types/lodash@4.17.24** — `MIT`
  - https://github.com/DefinitelyTyped/DefinitelyTyped
- **@types/mdast@4.0.4** — `MIT`
  - https://github.com/DefinitelyTyped/DefinitelyTyped
- **@types/ms@2.1.0** — `MIT`
  - https://github.com/DefinitelyTyped/DefinitelyTyped
- **@types/trusted-types@2.0.7** — `MIT`
  - https://github.com/DefinitelyTyped/DefinitelyTyped
- **@types/unist@3.0.3** — `MIT`
  - https://github.com/DefinitelyTyped/DefinitelyTyped
- **@vue/compiler-core@3.5.34** — `MIT` — Evan You
  - https://github.com/vuejs/core
- **@vue/compiler-dom@3.5.34** — `MIT` — Evan You
  - https://github.com/vuejs/core
- **@vue/compiler-sfc@3.5.34** — `MIT` — Evan You
  - https://github.com/vuejs/core
- **@vue/compiler-ssr@3.5.34** — `MIT` — Evan You
  - https://github.com/vuejs/core
- **@vue/reactivity@3.5.34** — `MIT` — Evan You
  - https://github.com/vuejs/core
- **@vue/runtime-core@3.5.34** — `MIT` — Evan You
  - https://github.com/vuejs/core
- **@vue/runtime-dom@3.5.34** — `MIT` — Evan You
  - https://github.com/vuejs/core
- **@vue/server-renderer@3.5.34** — `MIT` — Evan You
  - https://github.com/vuejs/core
- **@vue/shared@3.5.34** — `MIT` — Evan You
  - https://github.com/vuejs/core
- **ajv-formats@2.1.1** — `MIT` — Evgeny Poberezkin
  - https://github.com/ajv-validator/ajv-formats
- **ajv@8.20.0** — `MIT` — Evgeny Poberezkin
  - https://github.com/ajv-validator/ajv
- **atomically@1.7.0** — `MIT` — Fabio Spampinato
  - https://github.com/fabiospampinato/atomically
- **bail@2.0.2** — `MIT` — Titus Wormer
  - https://github.com/wooorm/bail
- **binary-extensions@2.3.0** — `MIT` — Sindre Sorhus
  - https://github.com/sindresorhus/binary-extensions
- **braces@3.0.3** — `MIT` — Jon Schlinkert
  - https://github.com/micromatch/braces
- **builder-util-runtime@9.5.1** — `MIT` — Vladimir Krivosheev
  - https://github.com/electron-userland/electron-builder
- **ccount@2.0.1** — `MIT` — Titus Wormer
  - https://github.com/wooorm/ccount
- **character-entities@2.0.2** — `MIT` — Titus Wormer
  - https://github.com/wooorm/character-entities
- **chokidar@3.6.0** — `MIT` — Paul Miller
  - https://github.com/paulmillr/chokidar
- **clsx@2.1.1** — `MIT` — Luke Edwards
  - https://github.com/lukeed/clsx
- **codemirror@6.0.2** — `MIT` — Marijn Haverbeke
  - https://github.com/codemirror/basic-setup
- **commander@8.3.0** — `MIT` — TJ Holowaychuk
  - https://github.com/tj/commander.js
- **conf@10.2.0** — `MIT` — Sindre Sorhus
  - https://github.com/sindresorhus/conf
- **core-util-is@1.0.2** — `MIT` — Isaac Z. Schlueter
  - https://github.com/isaacs/core-util-is
- **crelt@1.0.6** — `MIT` — Marijn Haverbeke
  - https://github.com/marijnh/crelt
- **csstype@3.2.3** — `MIT` — Fredrik Nicol
  - https://github.com/frenic/csstype
- **debounce-fn@4.0.0** — `MIT` — Sindre Sorhus
  - https://github.com/sindresorhus/debounce-fn
- **debug@4.4.3** — `MIT` — Josh Junon
  - https://github.com/debug-js/debug
- **decode-named-character-reference@1.3.0** — `MIT` — Titus Wormer
  - https://github.com/wooorm/decode-named-character-reference
- **dequal@2.0.3** — `MIT` — Luke Edwards
  - https://github.com/lukeed/dequal
- **devlop@1.1.0** — `MIT` — Titus Wormer
  - https://github.com/wooorm/devlop
- **dot-prop@6.0.1** — `MIT` — Sindre Sorhus
  - https://github.com/sindresorhus/dot-prop
- **electron-store@8.2.0** — `MIT` — Sindre Sorhus
  - https://github.com/sindresorhus/electron-store
- **electron-updater@6.8.3** — `MIT` — Vladimir Krivosheev
  - https://github.com/electron-userland/electron-builder
- **env-paths@2.2.1** — `MIT` — Sindre Sorhus
  - https://github.com/sindresorhus/env-paths
- **escape-string-regexp@5.0.0** — `MIT` — Sindre Sorhus
  - https://github.com/sindresorhus/escape-string-regexp
- **estree-walker@2.0.2** — `MIT` — Rich Harris
  - https://github.com/Rich-Harris/estree-walker
- **extend@3.0.2** — `MIT` — Stefan Thomas
  - https://github.com/justmoon/node-extend
- **fast-deep-equal@3.1.3** — `MIT` — Evgeny Poberezkin
  - https://github.com/epoberezkin/fast-deep-equal
- **fill-range@7.1.1** — `MIT` — Jon Schlinkert
  - https://github.com/jonschlinkert/fill-range
- **find-up@3.0.0** — `MIT` — Sindre Sorhus
  - https://github.com/sindresorhus/find-up
- **fs-extra@10.1.0** — `MIT` — JP Richardson
  - https://github.com/jprichardson/node-fs-extra
- **fsevents@2.3.3** — `MIT`
  - https://github.com/fsevents/fsevents
- **gemoji@8.1.0** — `MIT` — Titus Wormer
  - https://github.com/wooorm/gemoji
- **immediate@3.0.6** — `MIT`
  - https://github.com/calvinmetcalf/immediate
- **is-binary-path@2.1.0** — `MIT` — Sindre Sorhus
  - https://github.com/sindresorhus/is-binary-path
- **is-extglob@2.1.1** — `MIT` — Jon Schlinkert
  - https://github.com/jonschlinkert/is-extglob
- **is-glob@4.0.3** — `MIT` — Jon Schlinkert
  - https://github.com/micromatch/is-glob
- **is-number@7.0.0** — `MIT` — Jon Schlinkert
  - https://github.com/jonschlinkert/is-number
- **is-obj@2.0.0** — `MIT` — Sindre Sorhus
  - https://github.com/sindresorhus/is-obj
- **is-plain-obj@4.1.0** — `MIT` — Sindre Sorhus
  - https://github.com/sindresorhus/is-plain-obj
- **isarray@1.0.0** — `MIT` — Julian Gruber
  - https://github.com/juliangruber/isarray
- **js-yaml@4.1.1** — `MIT` — Vladimir Zapparov
  - https://github.com/nodeca/js-yaml
- **json-schema-traverse@1.0.0** — `MIT` — Evgeny Poberezkin
  - https://github.com/epoberezkin/json-schema-traverse
- **jsonfile@6.2.1** — `MIT` — JP Richardson
  - https://github.com/jprichardson/node-jsonfile
- **katex@0.16.47** — `MIT`
  - https://github.com/KaTeX/KaTeX
- **lazy-val@1.0.5** — `MIT` — Vladimir Krivosheev
  - https://github.com/develar/lazy-val
- **lie@3.3.0** — `MIT`
  - https://github.com/calvinmetcalf/lie
- **linkify-it@5.0.0** — `MIT`
  - https://github.com/markdown-it/linkify-it
- **locate-path@3.0.0** — `MIT` — Sindre Sorhus
  - https://github.com/sindresorhus/locate-path
- **lodash-es@4.18.1** — `MIT` — John-David Dalton
  - https://github.com/lodash/lodash
- **lodash.escaperegexp@4.1.2** — `MIT` — John-David Dalton
  - https://github.com/lodash/lodash
- **lodash.isequal@4.5.0** — `MIT` — John-David Dalton
  - https://github.com/lodash/lodash
- **longest-streak@3.1.0** — `MIT` — Titus Wormer
  - https://github.com/wooorm/longest-streak
- **magic-string@0.30.21** — `MIT` — Rich Harris
  - https://github.com/Rich-Harris/magic-string
- **markdown-it-emoji@3.0.0** — `MIT`
  - https://github.com/markdown-it/markdown-it-emoji
- **markdown-it@14.1.1** — `MIT`
  - https://github.com/markdown-it/markdown-it
- **markdown-table@3.0.4** — `MIT` — Titus Wormer
  - https://github.com/wooorm/markdown-table
- **marked@14.0.0** — `MIT` — Christopher Jeffrey
  - https://github.com/markedjs/marked
- **mdast-util-definitions@6.0.0** — `MIT` — Titus Wormer
  - https://github.com/syntax-tree/mdast-util-definitions
- **mdast-util-find-and-replace@3.0.2** — `MIT` — Titus Wormer
  - https://github.com/syntax-tree/mdast-util-find-and-replace
- **mdast-util-from-markdown@2.0.3** — `MIT` — Titus Wormer
  - https://github.com/syntax-tree/mdast-util-from-markdown
- **mdast-util-gfm-autolink-literal@2.0.1** — `MIT` — Titus Wormer
  - https://github.com/syntax-tree/mdast-util-gfm-autolink-literal
- **mdast-util-gfm-footnote@2.1.0** — `MIT` — Titus Wormer
  - https://github.com/syntax-tree/mdast-util-gfm-footnote
- **mdast-util-gfm-strikethrough@2.0.0** — `MIT` — Titus Wormer
  - https://github.com/syntax-tree/mdast-util-gfm-strikethrough
- **mdast-util-gfm-table@2.0.0** — `MIT` — Titus Wormer
  - https://github.com/syntax-tree/mdast-util-gfm-table
- **mdast-util-gfm-task-list-item@2.0.0** — `MIT` — Titus Wormer
  - https://github.com/syntax-tree/mdast-util-gfm-task-list-item
- **mdast-util-gfm@3.1.0** — `MIT` — Titus Wormer
  - https://github.com/syntax-tree/mdast-util-gfm
- **mdast-util-math@3.0.0** — `MIT` — Titus Wormer
  - https://github.com/syntax-tree/mdast-util-math
- **mdast-util-phrasing@4.1.0** — `MIT` — Victor Felder
  - https://github.com/syntax-tree/mdast-util-phrasing
- **mdast-util-to-markdown@2.1.2** — `MIT` — Titus Wormer
  - https://github.com/syntax-tree/mdast-util-to-markdown
- **mdast-util-to-string@4.0.0** — `MIT` — Titus Wormer
  - https://github.com/syntax-tree/mdast-util-to-string
- **mdurl@2.0.0** — `MIT`
  - https://github.com/markdown-it/mdurl
- **micromark-core-commonmark@2.0.3** — `MIT` — Titus Wormer
  - https://github.com/micromark/micromark.git#main
- **micromark-extension-gfm-autolink-literal@2.1.0** — `MIT` — Titus Wormer
  - https://github.com/micromark/micromark-extension-gfm-autolink-literal
- **micromark-extension-gfm-footnote@2.1.0** — `MIT` — Titus Wormer
  - https://github.com/micromark/micromark-extension-gfm-footnote
- **micromark-extension-gfm-strikethrough@2.1.0** — `MIT` — Titus Wormer
  - https://github.com/micromark/micromark-extension-gfm-strikethrough
- **micromark-extension-gfm-table@2.1.1** — `MIT` — Titus Wormer
  - https://github.com/micromark/micromark-extension-gfm-table
- **micromark-extension-gfm-tagfilter@2.0.0** — `MIT` — Titus Wormer
  - https://github.com/micromark/micromark-extension-gfm-tagfilter
- **micromark-extension-gfm-task-list-item@2.1.0** — `MIT` — Titus Wormer
  - https://github.com/micromark/micromark-extension-gfm-task-list-item
- **micromark-extension-gfm@3.0.0** — `MIT` — Titus Wormer
  - https://github.com/micromark/micromark-extension-gfm
- **micromark-extension-math@3.1.0** — `MIT` — Titus Wormer
  - https://github.com/micromark/micromark-extension-math
- **micromark-factory-destination@2.0.1** — `MIT` — Titus Wormer
  - https://github.com/micromark/micromark.git#main
- **micromark-factory-label@2.0.1** — `MIT` — Titus Wormer
  - https://github.com/micromark/micromark.git#main
- **micromark-factory-space@2.0.1** — `MIT` — Titus Wormer
  - https://github.com/micromark/micromark.git#main
- **micromark-factory-title@2.0.1** — `MIT` — Titus Wormer
  - https://github.com/micromark/micromark.git#main
- **micromark-factory-whitespace@2.0.1** — `MIT` — Titus Wormer
  - https://github.com/micromark/micromark.git#main
- **micromark-util-character@2.1.1** — `MIT` — Titus Wormer
  - https://github.com/micromark/micromark.git#main
- **micromark-util-chunked@2.0.1** — `MIT` — Titus Wormer
  - https://github.com/micromark/micromark.git#main
- **micromark-util-classify-character@2.0.1** — `MIT` — Titus Wormer
  - https://github.com/micromark/micromark.git#main
- **micromark-util-combine-extensions@2.0.1** — `MIT` — Titus Wormer
  - https://github.com/micromark/micromark.git#main
- **micromark-util-decode-numeric-character-reference@2.0.2** — `MIT` — Titus Wormer
  - https://github.com/micromark/micromark.git#main
- **micromark-util-decode-string@2.0.1** — `MIT` — Titus Wormer
  - https://github.com/micromark/micromark.git#main
- **micromark-util-encode@2.0.1** — `MIT` — Titus Wormer
  - https://github.com/micromark/micromark.git#main
- **micromark-util-html-tag-name@2.0.1** — `MIT` — Titus Wormer
  - https://github.com/micromark/micromark.git#main
- **micromark-util-normalize-identifier@2.0.1** — `MIT` — Titus Wormer
  - https://github.com/micromark/micromark.git#main
- **micromark-util-resolve-all@2.0.1** — `MIT` — Titus Wormer
  - https://github.com/micromark/micromark.git#main
- **micromark-util-sanitize-uri@2.0.1** — `MIT` — Titus Wormer
  - https://github.com/micromark/micromark.git#main
- **micromark-util-subtokenize@2.1.0** — `MIT` — Titus Wormer
  - https://github.com/micromark/micromark.git#main
- **micromark-util-symbol@2.0.1** — `MIT` — Titus Wormer
  - https://github.com/micromark/micromark.git#main
- **micromark-util-types@2.0.2** — `MIT` — Titus Wormer
  - https://github.com/micromark/micromark.git#main
- **micromark@4.0.2** — `MIT` — Titus Wormer
  - https://github.com/micromark/micromark.git#main
- **mimic-fn@2.1.0** — `MIT` — Sindre Sorhus
  - https://github.com/sindresorhus/mimic-fn
- **mimic-fn@3.1.0** — `MIT` — Sindre Sorhus
  - https://github.com/sindresorhus/mimic-fn
- **monaco-editor@0.55.1** — `MIT` — Microsoft Corporation
  - https://github.com/microsoft/monaco-editor
- **ms@2.1.3** — `MIT`
  - https://github.com/vercel/ms
- **nanoid@3.3.12** — `MIT` — Andrey Sitnik
  - https://github.com/ai/nanoid
- **nanoid@5.1.11** — `MIT` — Andrey Sitnik
  - https://github.com/ai/nanoid
- **normalize-path@3.0.0** — `MIT` — Jon Schlinkert
  - https://github.com/jonschlinkert/normalize-path
- **onetime@5.1.2** — `MIT` — Sindre Sorhus
  - https://github.com/sindresorhus/onetime
- **orderedmap@2.1.1** — `MIT` — Marijn Haverbeke
  - https://github.com/marijnh/orderedmap
- **p-limit@2.3.0** — `MIT` — Sindre Sorhus
  - https://github.com/sindresorhus/p-limit
- **p-locate@3.0.0** — `MIT` — Sindre Sorhus
  - https://github.com/sindresorhus/p-locate
- **p-try@2.2.0** — `MIT` — Sindre Sorhus
  - https://github.com/sindresorhus/p-try
- **path-exists@3.0.0** — `MIT` — Sindre Sorhus
  - https://github.com/sindresorhus/path-exists
- **picomatch@2.3.2** — `MIT` — Jon Schlinkert
  - https://github.com/micromatch/picomatch
- **pkg-up@3.1.0** — `MIT` — Sindre Sorhus
  - https://github.com/sindresorhus/pkg-up
- **postcss@8.5.15** — `MIT` — Andrey Sitnik
  - https://github.com/postcss/postcss
- **process-nextick-args@2.0.1** — `MIT`
  - https://github.com/calvinmetcalf/process-nextick-args
- **prosemirror-changeset@2.4.1** — `MIT`
  - git+https://code.haverbeke.berlin/prosemirror/prosemirror-changeset
- **prosemirror-commands@1.7.1** — `MIT`
  - https://github.com/prosemirror/prosemirror-commands
- **prosemirror-drop-indicator@0.1.3** — `MIT` — ocavue
  - https://github.com/prosekit/prosemirror-drop-indicator
- **prosemirror-dropcursor@1.8.2** — `MIT`
  - https://github.com/prosemirror/prosemirror-dropcursor
- **prosemirror-gapcursor@1.4.1** — `MIT`
  - https://github.com/prosemirror/prosemirror-gapcursor
- **prosemirror-history@1.5.0** — `MIT`
  - https://github.com/prosemirror/prosemirror-history
- **prosemirror-inputrules@1.5.1** — `MIT`
  - https://github.com/prosemirror/prosemirror-inputrules
- **prosemirror-keymap@1.2.3** — `MIT`
  - https://github.com/prosemirror/prosemirror-keymap
- **prosemirror-model@1.25.7** — `MIT`
  - git+https://code.haverbeke.berlin/prosemirror/prosemirror-model
- **prosemirror-safari-ime-span@1.0.2** — `MIT` — ocavue
  - https://github.com/ocavue/prosemirror-safari-ime-span
- **prosemirror-schema-list@1.5.1** — `MIT`
  - https://github.com/prosemirror/prosemirror-schema-list
- **prosemirror-state@1.4.4** — `MIT`
  - https://github.com/prosemirror/prosemirror-state
- **prosemirror-tables@1.8.5** — `MIT`
  - https://github.com/ProseMirror/prosemirror-tables
- **prosemirror-transform@1.12.0** — `MIT`
  - https://github.com/prosemirror/prosemirror-transform
- **prosemirror-view@1.41.8** — `MIT`
  - git+https://code.haverbeke.berlin/prosemirror/prosemirror-view
- **prosemirror-virtual-cursor@0.4.2** — `MIT` — ocavue
  - https://github.com/ocavue/prosemirror-virtual-cursor
- **punycode.js@2.3.1** — `MIT` — Mathias Bynens
  - https://github.com/mathiasbynens/punycode.js
- **react-dom@19.2.6** — `MIT`
  - https://github.com/facebook/react
- **react@19.2.6** — `MIT`
  - https://github.com/facebook/react
- **readable-stream@2.3.8** — `MIT`
  - https://github.com/nodejs/readable-stream
- **readdirp@3.6.0** — `MIT` — Thorsten Lorenz
  - https://github.com/paulmillr/readdirp
- **remark-gfm@4.0.1** — `MIT` — Titus Wormer
  - https://github.com/remarkjs/remark-gfm
- **remark-inline-links@7.0.0** — `MIT` — Titus Wormer
  - https://github.com/remarkjs/remark-inline-links
- **remark-math@6.0.0** — `MIT` — Junyoung Choi
  - https://github.com/remarkjs/remark-math.git#main
- **remark-parse@11.0.0** — `MIT` — Titus Wormer
  - https://github.com/remarkjs/remark.git#main
- **remark-stringify@11.0.0** — `MIT` — Titus Wormer
  - https://github.com/remarkjs/remark.git#main
- **remark@15.0.1** — `MIT` — Titus Wormer
  - https://github.com/remarkjs/remark.git#main
- **require-from-string@2.0.2** — `MIT` — Vsevolod Strukchinsky
  - https://github.com/floatdrop/require-from-string
- **rope-sequence@1.3.4** — `MIT` — Marijn Haverbeke
  - https://github.com/marijnh/rope-sequence
- **safe-buffer@5.1.2** — `MIT` — Feross Aboukhadijeh
  - https://github.com/feross/safe-buffer
- **scheduler@0.27.0** — `MIT`
  - https://github.com/facebook/react
- **setimmediate@1.0.5** — `MIT` — YuzuJS
  - https://github.com/YuzuJS/setImmediate
- **state-local@1.0.7** — `MIT` — Suren Atoyan
  - https://github.com/suren-atoyan/state-local
- **string_decoder@1.1.1** — `MIT`
  - https://github.com/nodejs/string_decoder
- **style-mod@4.1.3** — `MIT` — Marijn Haverbeke
  - https://github.com/marijnh/style-mod
- **tiny-typed-emitter@2.1.0** — `MIT` — Zurab Benashvili
  - https://github.com/binier/tiny-typed-emitter
- **to-regex-range@5.0.1** — `MIT` — Jon Schlinkert
  - https://github.com/micromatch/to-regex-range
- **trough@2.2.0** — `MIT` — Titus Wormer
  - https://github.com/wooorm/trough
- **turndown-plugin-gfm@1.0.2** — `MIT` — Dom Christie
  - https://github.com/domchristie/turndown-plugin-gfm
- **turndown@7.2.4** — `MIT` — Dom Christie
  - https://github.com/mixmark-io/turndown
- **uc.micro@2.1.0** — `MIT`
  - https://github.com/markdown-it/uc.micro
- **unified@11.0.5** — `MIT` — Titus Wormer
  - https://github.com/unifiedjs/unified
- **unist-util-is@6.0.1** — `MIT` — Titus Wormer
  - https://github.com/syntax-tree/unist-util-is
- **unist-util-remove-position@5.0.0** — `MIT` — Titus Wormer
  - https://github.com/syntax-tree/unist-util-remove-position
- **unist-util-stringify-position@4.0.0** — `MIT` — Titus Wormer
  - https://github.com/syntax-tree/unist-util-stringify-position
- **unist-util-visit-parents@6.0.2** — `MIT` — Titus Wormer
  - https://github.com/syntax-tree/unist-util-visit-parents
- **unist-util-visit@5.1.0** — `MIT` — Titus Wormer
  - https://github.com/syntax-tree/unist-util-visit
- **universalify@2.0.1** — `MIT` — Ryan Zimmerman
  - https://github.com/RyanZim/universalify
- **util-deprecate@1.0.2** — `MIT` — Nathan Rajlich
  - https://github.com/TooTallNate/util-deprecate
- **vfile-message@4.0.3** — `MIT` — Titus Wormer
  - https://github.com/vfile/vfile-message
- **vfile@6.0.3** — `MIT` — Titus Wormer
  - https://github.com/vfile/vfile
- **vue@3.5.34** — `MIT` — Evan You
  - https://github.com/vuejs/core
- **w3c-keyname@2.2.8** — `MIT` — Marijn Haverbeke
  - https://github.com/marijnh/w3c-keyname
- **zwitch@2.0.4** — `MIT` — Titus Wormer
  - https://github.com/wooorm/zwitch

### OFL-1.1 (2)

- **@fontsource/open-sans@5.2.7** — `OFL-1.1` — Google Inc.
  - https://github.com/fontsource/font-files
- **@fontsource/source-serif-pro@5.2.5** — `OFL-1.1` — Google Inc.
  - https://github.com/fontsource/font-files

### Python-2.0 (1)

- **argparse@2.0.1** — `Python-2.0`
  - https://github.com/nodeca/argparse
